...
public class LottoBase {
    ...
    private void MakeNewRandomWinningNumbers() {
        ...
        for (int x = 0; x < 6; x++) {
            ...
            SingleNumberView TempView = new SingleNumberView();
            SingleNumberController TempController = new SingleNumberController(TempModel, TempView);
            TempController.setNumber(randomNumber);

            for (int i = 0; i < lastWinningNumbers.size(); i++) {
                if (lastWinningNumbers.get(i).getNumber() == TempController.getNumer()) {
                    randomNumber = rand.nextInt(40) + 1;
                    TempController.setNumber(randomNumber);
                    i--;
                }
     ...
     * @param args is an additional amount or amounts of sets to generate
     */
    public void DoNewRandomWinningSetsProcedure(int amount, int... args) throws NumberFormatException {
    ...
    public void getDataFromDatabase() throws LottoBaseException, FileNotFoundException {
        ...
        while (Reader.hasNextLine()) {
            ...
            TempController.setNumber(Reader.nextInt());

            if (TempController.getNumer() <= 40 && TempController.getNumer() >= 1) {
